...
public class UserStoreTest {
	...
	private final User USER_ONE = new User(UUID.randomUUID(),
			"test_username_one",
			"$2a$10$/zf4WlT2Z6tB5sULB9Wec.QQdawmF0f1SbqBw5EeJg5uoVpKFFXAa",
			Instant.ofEpochMilli(1000));
	private final User USER_TWO = new User(UUID.randomUUID(),
			"test_username_two",
			"$2a$10$lgZSbmcYyyC7bETcMo/O1uUltWYDK3DW1lrEjCumOE1u8QPMlzNVy",
			Instant.ofEpochMilli(2000));
	private final User USER_THREE = new User(UUID.randomUUID(),
			"test_username_three",
	...
	private void assertEquals(User expectedUser, User actualUser) {
		...
		Assert.assertEquals(expectedUser.getCreationTime(),
				actualUser.getCreationTime());
	}
}
